...
import * as useMutations from "@/app/(auth)/sing-in/_services/use-mutations";

vi.mock("@/app/(auth)/sing-in/_services/use-mutations");
...
vi.mock("@/app/(auth)/sing-in/_types/sing-in-schema", () => ({
...
const mockUseSingIn = vi.mocked(useMutations.useSingIn);
...
describe("SingInForm", () => {
  beforeEach(() => {
    ...
    mockUseSingIn.mockReturnValue({
      mutate: mockMutate,
      isPending: false,
    } as unknown as ReturnType<typeof useMutations.useSingIn>);
  });
  ...
  it("test_renders_all_form_elements_correctly", () => {
    ...
    expect(
      screen.getByRole("button", { name: /sing in/i }),
    ).toBeInTheDocument();
  ...
  it("test_navigates_to_sign_up_page_on_link_click", () => {
    ...
    expect(signUpLink).toHaveAttribute("href", "/sing-up");
  });
