...
() => {
  ...
  it('Should Render Skeleton Elements With Correct CSS Classes For Styling', () => {
    ...
    const titleSkeletons = screen
      ...
      .filter((element) => element.className.includes('h-5 w-24'));
    expect(titleSkeletons).toHaveLength(1); // One per card

    const actionSkeletons = screen
      .getAllByTestId('skeleton')
      .filter((element) => element.className.includes('size-6'));
    expect(actionSkeletons).toHaveLength(2); // Two per card

    const labelSkeletons = screen
      .getAllByTestId('skeleton')
      .filter((element) => element.className.includes('mb-1 h-4'));
    expect(labelSkeletons).toHaveLength(4); // Four per card

    const valueSkeletons = screen
      .getAllByTestId('skeleton')
      .filter(
        (element) =>
          element.className.includes('h-4 w-12') &&
          !element.className.includes('mb-1'),
      );
    expect(valueSkeletons).toHaveLength(4); // Four per card
  });
  ...
  it('Should Display Proper Card Structure With Header, Separator, And Grid Layout', () => {
    ...
    const headerSections = screen
      .getAllByRole('generic')
      .filter((element) => element.className.includes('flex justify-between'));
    expect(headerSections).toHaveLength(1);
  });
  ...
  it('Should Render Action Buttons Placeholders (2 Skeleton Buttons Per Card)', () => {
    ...
    const actionContainers = screen
      .getAllByRole('generic')
      .filter((element) => element.className.includes('flex gap-1'));
    expect(actionContainers).toHaveLength(1);

    // Check for action button skeletons (size-6 class indicates button-sized skeletons)
    const actionButtonSkeletons = screen
      .getAllByTestId('skeleton')
      .filter((element) => element.className.includes('size-6'));
    expect(actionButtonSkeletons).toHaveLength(2); // 2 per card
  });
  ...
  it('Should Render Nutritional Info Grid With 4 Skeleton Sections Per Card', () => {
    ...
    const nutritionalSections = screen
      .getAllByRole('generic')
      .filter(
        (element) =>
    ...
    expect(nutritionalSections).toHaveLength(4); // 4 per card

    // Each nutritional section should have a label skeleton (mb-1 h-4)
    const nutritionalLabels = screen
      .getAllByTestId('skeleton')
      .filter((element) => element.className.includes('mb-1 h-4'));
    expect(nutritionalLabels).toHaveLength(4);

    // Each nutritional section should have a value skeleton (h-4 w-12 without mb-1)
    const nutritionalValues = screen
      .getAllByTestId('skeleton')
      .filter(
        (element) =>
          element.className.includes('h-4 w-12') &&
    ...
    expect(nutritionalValues).toHaveLength(4);
  });
});